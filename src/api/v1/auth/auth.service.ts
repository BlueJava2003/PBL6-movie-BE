...
export class AuthService {
    ...
    async refreshToken(refreshTokenDto:refreshTokenDTO): Promise<{ accessToken: string; refresh_token: string }>{
        try {
            const decodedToken = await this.jwtService.verifyAsync(refreshTokenDto.refreshToken, {
                secret: process.env.JWT_SECRET_KEY
              });
            const user = await this.prisma.auth.findUnique({
    ...
    async changePassword (data : changePasswordDTO,id:number):Promise<void>{
        try {
            ...
            const checkPassword = await bcrypt.compare(data.oldPassword,auth.password);
            if(!checkPassword)
